...
- [x] 1.1 在独立 `quality.gradle` 中接入与 Java 17、Gradle 8.8 兼容且版本固定的 Spotless 与 PMD 配置，限定 Java 源码范围并启用高优先级 `errorprone` 分析规则。
- [x] 1.2 在 `quality.gradle` 中添加 `qualityCheck` 聚合任务，使其执行格式检查和主/测试源码 PMD 检查，且不解析 ForgeGradle。
- [x] 1.3 执行 `./gradlew -b quality.gradle qualityCheck` 并验证成功；为格式或 PMD 故意制造一个可恢复的违规，确认任务失败且诊断可定位。

## 2. 本地提交门禁

- [x] 2.1 添加受版本控制的 `.githooks/pre-commit`，使其调用独立 `quality.gradle` 的 `qualityCheck`、在失败时中止提交并给出格式化或诊断提示。
...
- [x] 3.1 新增最小权限的 `.github/workflows/quality.yml`，使用 Java 17、Gradle Wrapper 与 Gradle 缓存执行独立 `quality.gradle` 的 `qualityCheck`。
